...
### I don't know why forward phase in White twinkle, and others isn't as smooth - hardware timing??? ###
...
LEDStripColorPhasing::LEDStripColorPhasing(LEDStripG35& g35, uint8_t pattern) : LEDStripLightProgram(g35, pattern), wait_(0), frequencyR_(0.06), frequencyG_(0.06), frequencyB_(0.06),
    phaseR_(0), phaseG_(2*PI/3), phaseB_(4*PI/3), center_(64), width_(63), fStep_(0), pStep_(0), fForward_(true), turn_(0), pattern_(pattern), grnOff_(false), bluOff_(false), redOff_(false)
{
}

// neat rainbow phase 0, 2*PI/3, 4*PI/3 (120deg = 2PI/3, so 0, 120, 240)

uint32_t LEDStripColorPhasing::Do()
{
    switch (pattern_ % 14)
    {
    ...
    for (int i=0; i < light_count_; i++)
    {
        ...
        g35_.fill_color(i, 1, COLOR(red, grn, blu));
    }
