...
LEDStripSimplexNoise::LEDStripSimplexNoise(LEDStripG35& g35, uint8_t pattern) : LEDStripLightProgram(g35, pattern), rMult_(1.0), gMult_(1.0), bMult_(1.0), spaceinc_(0.2), timeinc_(0.02), yoffset_(0.0), pattern_(pattern)
{
}
...
uint32_t LEDStripSimplexNoise::Do()
{
...
// LEDStrip: put happyinmotion's control constants (timeinc, spaceinc, yoffset, repeats) into function as parameters
    ...
    {
        xoffset += spaceinc_;
        bulbArray_red[i] = LEDStripSimplexNoise::SimplexNoise(xoffset, yoffset_, 0);
        bulbArray_green[i] = LEDStripSimplexNoise::SimplexNoise(xoffset, yoffset_, 1);
        bulbArray_blue[i] = LEDStripSimplexNoise::SimplexNoise(xoffset, yoffset_, 2);
    }
    ...
    for (int i = 0; i < light_count_; i++)
    {
        ...
        g35_.fill_color(i, 1, COLOR(int(r * rMult_), int(g * gMult_), int(b * bMult_)));
    }
	g35_.show();
    yoffset_ += timeinc_;
    return bulb_frame_;
...
int LEDStripSimplexNoise::fastfloor(float n)
{
    return n > 0 ? (int) n : (int) n - 1;
}

float LEDStripSimplexNoise::k_fn(int a)
{
...
int LEDStripSimplexNoise::shuffle(int i, int j, int k)
{
    return b(i, j, k, 0) + b(j, k, i, 1) + b(k, i, j, 2) + b(i, j, k, 3) + b(j, k, i, 4) + b(k, i, j, 5) + b(i, j, k, 6) + b(j, k, i, 7);
}

int LEDStripSimplexNoise::b(int i, int j, int k, int B)
{
    return T[b(i, B) << 2 | b(j, B) << 1 | b(k, B)];
}

int LEDStripSimplexNoise::b(int N, int B)
{
    return N >> B & 1;
}